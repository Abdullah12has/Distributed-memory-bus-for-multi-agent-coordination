...
This thesis studies that question on *multi-fragment LLM workflows*. The empirical evaluation uses either a deterministic regex parser or a single LLM call with all (compressed) fragments visible as the planner; multi-round agent simulation is outside the empirical scope. Four contributions are delivered: **(C1)** a reproducible coordination benchmark of 150 instances across three workload families — cross-document fact aggregation, constraint-satisfaction planning, and multi-step retrieval; **(C2)** an empirical characterisation of the *coordination cliff* $\tau^{*}$ across four training-free compressors (LLMLingua-2, Phi-3-Mini extractive, an instruction-aware filter, and a truncation baseline), with a compounding-error model that *explains the existence and sharpness* of the cliff and gives a *first-order* estimate of its position from per-round critical-token recall (median relative error $\approx 35\%$; the model's bootstrap band quantifies sampling rather than specification error and does not contain the empirical position — §4.4.4, §5.3), together with a cross-architecture consistency check on Qwen-2.5-72B (cliff position $7.3\%$ from the synthetic reference, inside its bootstrap CI) and DeepSeek V4 Pro (point estimate $\tau^{*}=2.15$, $13.8\%$ from the reference, but with a bootstrap CI so wide — $[1.76,7.14]$, with a bootstrap median of $5.7$ far from the point estimate — that the cliff position is only weakly identified at this sample size) inside a formally-defined calibrated regime; **(C3)** a catalogue of three RAG pipelines showing that, *on the single retriever/embedder stack evaluated here* (`bge-large-en-v1.5` + FAISS-CPU), compress-first is preferred over retrieve-first in both storage-bounded and accuracy-bounded regimes — a result that does not reproduce the post-retrieval-compression preference of the LongLLMLingua line of work on this stack (we do not claim to falsify it in general). A joint relevance-conditional pipeline scores higher F1 still, but the EUR cost model in its current form emits *identical* per-pipeline cost breakdowns (§4.7), so no cost-parity claim is made for that comparison; and **(C4)** a memory-bus reference implementation built around a tamper-evident audit log and a policy-aware compressor abstraction, together with a summary-level *inference-disclosure* metric that quantifies how compression reduces the recoverability of protected facts to a downstream reader.

Six hypotheses are tested across tens of thousands of evaluation cells (approximately $52\,000$ — see Chapter 4 for the per-experiment counts). **H1** (single-agent question-answering accuracy is not a transferable predictor of coordination success under compression) is supported: at the workload level (one measurement per workload, $n=150$; the earlier pooled $n=1350$ over-counted the nine ratios per workload as independent) Spearman $\rho \in [-0.82, +0.32]$ across compressors, with 95% bootstrap confidence intervals excluding 0.6 for all four. The striking finding is that the pooled correlation is largely an artifact: LLMLingua-2 and truncation, which look moderately positive when pooled ($\rho\approx+0.38$), collapse to near-zero at the workload level ($\rho=+0.03$ and $+0.05$, CIs spanning zero), while the instruction-aware filter is strongly negative ($\rho=-0.82$) and Phi-3-Mini extractive moderately positive ($\rho=+0.32$). The filter's overall negative value is moreover a *between-family* effect: within each family it is weak or undefined (family-a $\rho=-0.13$, n.s.; families b and c degenerate because coordination collapses uniformly, leaving no rank variance). Across the twelve (compressor, family) cells the only statistically significant within-family workload-level correlation is *negative* (LLMLingua-2 on family-c, $\rho=-0.46$, $p=7\times10^{-4}$). The honest H1 conclusion is therefore not "compressors disagree on a consistent sign" but the stronger methodological point that QA-F1 change does not positively predict coordination change for any compressor at the workload level, and that the pooled single-number correlations are inflated/distorted by pseudo-replication and between-family pooling. **H2** (a sharp cliff exists) is supported on eleven of twelve (compressor, family) cells with paired Wilcoxon $p < 0.0001$. The original **H5** is reframed as **Corollary 1 (ceiling-cliff separation)**, stated as *no cliff-position shift was detected across the heterogeneous planners tested, on the cells where the cliff is resolvable* rather than as proven invariance. Model capacity sets the success ceiling $p_0$; the cliff position $\tau^{*}$ is set by the compressor and task threshold $\theta_q$. The local cross-architecture sweep at \{Qwen-2.5-1.5B, Phi-3-Mini-3.8B, Llama-3.1-8B\} shows a $24\%$ tau spread on family-c — within the $50\%$ tolerance the validator originally shipped with but *failing* the strict $20\%$ tolerance that matches the H2 spec. The strongest single test is therefore one frontier cell: Qwen-2.5-72B cliffs $7.3\%$ from the canonical synthetic reference $\tau^{*}=2.5$ (inside its bootstrap CI). DeepSeek V4 Pro's point estimate is $\tau^{*}=2.15$ ($13.8\%$ below the reference), but its bootstrap CI is $[1.76,7.14]$ with a bootstrap *median* of $5.7$ — far from the $2.15$ point estimate — so the cliff position is only weakly identified and CI-containment of the reference is weak evidence (it is not an equivalence test, and the CI would contain almost any cliff position in the swept range). The corollary should be read as supported on one reasonably-identified cell (Qwen-72B), consistent on the local arm, and not yet established at the strict tolerance. The original **H6** is reframed and supported as **Corollary 2 (information-density scaling)**: the *shape* of the cliff (sharpness and pre-cliff plateau) varies with task information density, with $\theta_\text{info}=0.97$ on a dense numeric-aggregation family and $0.37$–$0.48$ on distributed multi-document benchmarks (HotpotQA and MultiHopRAG). **H4** (compression reduces inference disclosure) is supported for three of four compressors with two documented caveats: (i) the Llama-3.1-8B reader is YES-biased so the metric tests asymmetric leakage; (ii) the disclosure-reduction ranking is monotonic in *how aggressively each compressor destroys source tokens* — truncation (most destructive) leads at $24.6$ pp, the token-level filter and LLMLingua-2 follow at $20.4$ and $18.9$ pp, and Phi-3-Mini extractive (verbatim-span preserving) shows no significant reduction ($0.4$ pp, $p = 0.91$). H4 thus measures information destruction rather than privacy-specific filtering; the metric remains useful as a deployment-time disclosure-budget mechanism but the privacy interpretation is calibrated against the destruction baseline. The thesis also contributes **CAAC** — a cliff-aware adaptive compression wrapper that operationalises the compounding-error bound as a runtime back-off, populating a complementary operating point on the (coordination, ratio) frontier.

The deliverable is the manuscript together with an open-source reference implementation. The local pipeline (H1, H2, H3, H4, H5) is reproducible from a single command on an Apple M4 Pro 48 GB laptop given a precomputed compression cache; the cache itself is GPU-bound and requires an RTX 5090 32 GB workstation (≈14 h). The frontier-API arm (Qwen-2.5-72B, DeepSeek V4 Pro, GPT-oss 120B) requires paid API credentials and is *not* byte-deterministic — frontier-server batching and model updates make exact CSV reproduction impossible; the published `frontier_*/results.csv` files are the canonical record. All local CSVs are byte-deterministic given the seed.
...
The statistical protocol is fixed in advance for every hypothesis. Effect-size summaries are computed at the workload level, never the cell level: per-workload means are computed first, and 95% bootstrap confidence intervals are constructed by resampling workloads with replacement [Efron 1993], never by resampling individual seed–cell pairs. The bootstrap method is the bias-corrected and accelerated (BCa) interval [Efron 1987] with $n_\text{boot} = 1000$ resamples, except where noted otherwise (the $\theta_q$ resampling pipeline of §4.4.4 also uses BCa with $n_\text{boot} = 1000$). The two-cell comparison test is the paired Wilcoxon signed-rank test [Wilcoxon 1945], chosen over the Mann–Whitney $U$ test [Mann 1947] because the same workloads appear in both conditions, which violates the independence assumption of the two-sample test. The original cliff machinery used Mann–Whitney $U$; that error was corrected during the audit pass and the canonical `verdicts.json` files reflect the paired-Wilcoxon recomputation. Multiple-comparisons correction is Holm's sequentially-rejective procedure [Holm 1979] applied *within each hypothesis's family*: the 12 (compressor, family) cells of the cliff sweep are Holm-corrected jointly for H2; the three condition pairs of H4 are Holm-corrected jointly for the disclosure signal and again jointly for the disclosure reduction; the three pipelines of H3 are Holm-corrected jointly per regime. Family sizes are deliberately kept narrow because the hypotheses are independent claims and joint correction across them would be over-conservative.

The H1 verdict statistic is the *workload-level* Spearman correlation ($n=150$ workloads, one mean $\Delta$-pair per workload; $n=100$ for Phi-3-Mini extractive after its $\sim 2.5\times$ achievable-compression ceiling collapses the upper ratio range, §3.2.3). An earlier draft reported a pooled correlation over $n=150\times 9$ (workload, ratio) measurements; because the nine per-workload ratios are repeated measures rather than independent draws, that pooled $n$ and its asymptotic $p$-value over-state significance by roughly an order of magnitude and are retained in Table 4.1 only as a non-inferential comparison column. The two-tailed asymptotic $p$ for $H_0: \rho=0$ is reported at the workload level; the verdict rests on the BCa bootstrap CI (over workloads) excluding 0.6.
...
The sweep uses **10 family-a workloads at 6 sweep ratios with 3 seeds per cell, for a total of 180 cells per frontier model** (the original setup-paragraph claim of "30 workloads × 10 ratios × 5 seeds = 1500" was an error; the canonical CSVs under `results/frontier_*/results.csv` each contain 180 rows). Planner inference is via the OpenAI-compatible Featherless endpoint for Qwen-2.5-72B and DeepSeek V4 Pro and via the OpenAI Responses API for GPT-oss 120B. $\tau^{*}$ is fit by the same paired-fit-and-select procedure as the local sweep, and the bootstrap CI is constructed by resampling workloads ($n_\text{boot} = 500$). The synthetic reference is the family-a × LLMLingua-2 value $\tau^{*}_\text{canonical} = 2.5$ from `results/h1_h2_v2/verdicts.json` (the canonical logistic fit selected by the paired-fit-and-select procedure). A separate auxiliary fit on the original `h1_h2_final` family-a coordination curve gives $\tau^{*}_\text{aux} = 2.6997$; both numbers appear in the project's audit-trail JSON because the frontier runner was written against `h1_h2_final` before `h1_h2_v2` superseded it. The thesis stands by the canonical 2.5 fit; we report the comparison against the auxiliary 2.6997 parenthetically where it appears in the verdicts JSON. The canonical results directories are `results/frontier_qwen72b/` for Qwen, `results/frontier_deepseekv4/` for DeepSeek, and `results/frontier_gptoss120b/` (with `STATUS_NONCANONICAL.txt`) for GPT-oss.
...
The Qwen-2.5-72B sweep returns $\tau^{*} = 2.68$ (bootstrap CI $[1.92, 7.23]$). The canonical family-a × LLMLingua-2 synthetic reference from `results/h1_h2_v2/verdicts.json` is $\tau^{*}_\text{canonical} = 2.5$ (the selected logistic fit; see §4.3 Table 4.2); the Qwen-72B point estimate is **$\sim 7\%$ above** this reference and the bootstrap CI comfortably contains it. (A separate auxiliary fit on the original `h1_h2_final` curve produces $\tau^{*}_\text{aux} = 2.6997$, against which Qwen-72B is $0.8\%$ off; this is the number quoted in the project's earlier audit-trail JSON but is not the canonical fit. The "$\sim 7\%$ off canonical, CI contains canonical" framing is the one this thesis stands by.) This is the most direct evidence for Corollary 1's *within-family parameter scaling* claim: same model family (Qwen-2.5), same compressor (LLMLingua-2), a **48× change in parameter count** from local 1.5B Qwen-2.5 to frontier 72B Qwen-2.5, and the cliff position falls in the same 2--3$\times$ ratio band with the bootstrap CI containing the canonical reference. (The 48× scope is the local-to-frontier Qwen-2.5 jump on the family-a × LLMLingua-2 cell; cross-family or cross-compressor scaling of the 72B planner is not part of the sweep.) The baseline coordination $p_0(\text{Qwen-72B}) = 1.00$ is at the ceiling, which places the model squarely inside the calibrated regime: there is no floor effect to argue around.

### 4.6.3 DeepSeek V4 Pro: within tolerance but weakly identified

The DeepSeek V4 Pro sweep returns a $\tau^{*}$ point estimate of 2.15 — $13.8\%$ below the canonical synthetic reference $\tau^{*}_\text{canonical} = 2.5$, which is *inside* the strict $\pm 20\%$ tolerance — but with a bootstrap CI of $[1.76, 7.14]$ (width $5.4$ ratio units) and, tellingly, a bootstrap *median* of $5.7$ that sits far from the $2.15$ point estimate. That gap between point estimate and bootstrap median is the diagnostic: the cliff-fit is unstable under resampling (the curve admits both an early-cliff and a late-cliff fit on different resamples), so the position is only weakly identified. The honest framing is therefore: DeepSeek's point estimate agrees with the reference within tolerance, but the agreement is not robust — CI-containment of the reference is weak evidence when the CI spans most of the swept range, and is not an equivalence test. A formal TOST equivalence test against the $\pm 20\%$ band would not pass at this sample size (the CI extends to 7.14). Distinguishing a genuine within-tolerance match from an artifact of an unstable fit requires more seeds; the thesis takes the conservative reading that DeepSeek *corroborates* Corollary 1 without independently *confirming* it, and the bootstrap samples were not retained to permit the TOST post hoc (§5.3).
...
The compress-first preference over retrieve-first is a substantive finding because it does not reproduce the assumed post-retrieval-compression default on the stack we tested, and the P3-higher-F1 result points to a promising routing variant. For practitioners, the recommendation is deliberately hedged on cost: **compress-first (P1) is the safe default over retrieve-first (P2) on a comparable stack**; **P3 (joint relevance-conditional routing) is worth evaluating where the F1 gain matters** — its F1 leads compress-first by 42 pp (storage) and 25 pp (accuracy) — but note that the current cost model assigns all three pipelines identical EUR by construction (§4.7.4), so the F1 gain cannot yet be claimed as cost-free and must be re-measured with a faithful per-pipeline cost model before deployment; post-retrieval compression (P2) is only preferable when the corpus is small enough that storage cost is irrelevant and retrieval recall is the binding constraint.
...
A discriminating control — measuring privacy gain relative to coordination-utility loss — is named as future work in §5.6. The empirical implication for the memory bus's policy layer (§4.8.8) is that the disclosure-budget enforcement mechanism works as designed (more aggressive compression yields lower disclosure rates), but the *privacy* interpretation requires a target threat model that values destruction-induced reduction, not just compressor-specific reduction.
...
The cliff position shows **no detected shift** across the three local architecture families tested (Qwen-2.5-1.5B, Phi-3-Mini-3.8B, Llama-3.1-8B) within a precisely-defined calibrated regime (§4.5) — though the local arm's 24% spread fails the strict $\pm 20\%$ tolerance, so this arm is consistent-with but not sufficient-for the claim. The load-bearing evidence is a single frontier cell: on family-a × LLMLingua-2, Qwen-2.5-72B cliffs at $\tau^{*}=2.68$, $7.3\%$ from the canonical synthetic reference $\tau^{*}=2.5$ and inside its bootstrap CI — a 48× scale-up from the 1.5B planner with no detected shift. (Two honest qualifications: the local resolvable cells are family-c while this frontier cell is family-a, so the "48× scaling" spans different task families; and DeepSeek V4 Pro's point estimate ($\tau^{*}=2.15$, $13.8\%$ from the reference) sits within tolerance but inside a bootstrap CI so wide — $[1.76,7.14]$ with median $5.7$, far from the point estimate — that its cliff position is only weakly identified, so it corroborates without robustly confirming, §4.6.) The defensible claim is therefore "no shift detected on the resolvable cells, strongest on one frontier cell," not "invariant." The cliff is *not* invariant to reasoning regime: extended-reasoning planners such as GPT-oss 120B cliff at substantially higher ratios because they can recover from sub-threshold information by chain-of-thought, violating the threshold-success assumption A3. This is the empirical evidence that the cliff is a property of the (compressor, task) pair within a defined regime — the structural finding that the thesis contributes to the compression literature.
...
The $\theta_q$/$N$ ablation (`results/caac_theta_*`, `results/caac_N_*`) returns an informative null. The coordination-success plateau of CAAC is invariant to the choice of $\theta_q$ in $\{0.6, 0.7, 0.8\}$ and the choice of $N$ in $\{2, 3, 4, 5\}$: the LLMLingua-2 plateau is 33% across all seven configs, the filter plateau is 50% across all seven configs, and only the achieved compression ratio responds, by backing off more aggressively as either parameter increases. The primary knob is the $r_\text{min}$ floor, not $\theta_q$ or $N$. A sweep over $r_\text{min} \in \{1.2, 1.5, 2.0, 2.5\}$ is the natural follow-on that this thesis defers as future work.
...
**Predicted-$\tau^{*}$ bootstrap band is too tight.** The bootstrap CI on $\theta_q$ that the §4.4.4 predicted-$\tau^{*}$ band is built from quantifies *sampling* uncertainty only — how much the prediction would wobble if we resampled workloads. It does not quantify the model's *specification* error. The empirical $\tau^{*}$ falls *outside* this bootstrap band on $11/11$ testable cells (recorded in `insights.txt` §57). The right way to read the figure is "the band is tight because $\theta_q$ is well-estimated; the model is biased because A1–A4 are first-order approximations"; the actual accuracy of the model is what the in-sample/LOO match rates above measure.

**Corollary 1's local arm misses the strict tolerance.** The local Qwen-2.5 model-scaling sweep at $\{1.5, 3.8, 8\}$B on family-c shows a tau spread of approximately 24%, which is well below the 50% tolerance the `validate_model_independence` default originally shipped with but *above* the strict $\pm 20\%$ tolerance the audit recommended (aligning with the H2 falsifiable-claim spec). The local arm is therefore necessary but not sufficient for Corollary 1; the cross-architecture frontier evidence in §4.6 (Qwen-72B within 0.8% of the synthetic reference; DeepSeek V4 Pro bootstrap CI contains the reference) is what carries the corollary's binary verdict across the strict bar. The on-disk record is `results/h5_final/model_independence_20pct.json`.

**A3 (threshold success) is justified circularly.** A3 is licensed by the sharp empirical cliff of §4.3, and the cliff is in turn explained by A3 — the justification is circular. The cliff *exists* whether or not A3 is the right model of why; A3 is the *first-order* description that lets us write down a tractable bound. Breaking the circularity requires an independent A3 probe — for example, varying the surviving fraction of task-relevant tokens directly (e.g. by hand-curated token deletion) rather than mediating that variation through a compressor, and measuring whether success follows the same threshold–sigmoid shape. This is named as a future-work direction in §5.6.
...
**H3 cost model does not distinguish pipelines.** In the canonical `results/h3_final/results.csv`, `eur_per_query` is a single constant ($2.7\times10^{-5}$) on all 900 rows — identical across P1/P2/P3, both regimes, and all ratios — and retrieval-recall is likewise constant across regimes (§4.7). As a result the thesis makes no cost-parity claim for the P3-vs-P1 comparison, and the storage-bounded vs accuracy-bounded distinction is not reflected in the per-row metrics. A faithful per-pipeline cost model is required to attribute P3's F1 advantage to routing rather than to unmetered cost (§5.6).

**Frontier bootstrap samples not retained.** The frontier-API runs persist only summary CIs, not the per-resample bootstrap distribution, so a formal TOST equivalence test against the $\pm 20\%$ tolerance could not be computed; §4.6 reasons from the CIs instead and flags DeepSeek V4 Pro's CI-containment as underpowered (affirming-the-null) rather than as equivalence.
...
*First*, the cliff is a property of the (compressor, task) pair within the calibrated regime, not of the planner. The frontier validation results — Qwen-2.5-72B's $\tau^{*}=2.68$ sitting $\sim 7\%$ above the canonical synthetic reference $\tau^{*}=2.5$ and inside its bootstrap CI, plus invariance across three local architecture families (Qwen-2.5-1.5B, Phi-3-Mini-3.8B, Llama-3.1-8B) — are the empirical evidence. The implication for the compression literature is that future compressor evaluations should either characterise the calibrated regime they apply in or explicitly target the extended-reasoning regime as a separate case.
...
**Independent A3 probe (break the cliff–A3 circularity).** The cliff sharpness of §4.3 currently licenses A3, and A3 in turn explains the cliff — the justification is circular (§5.3). An independent A3 probe would directly vary the surviving fraction of task-relevant tokens without mediating through compression: hand-curated deletion of $k$ of the $M$ critical tokens at controlled fractions $k/M \in \{0.1, 0.2, \dots, 0.9\}$, then measure whether coordination success follows the threshold–sigmoid shape A3 predicts. A positive result would replace the circular justification with a direct one; a negative result would point at a graded-success refinement and feed into the extended-reasoning future-work item above.
...
The `make repro-all` target runs the eight steps above in dependency order. **Determinism scope:** all *local-pipeline* outputs (H1, H2, H3, H4, H5, the cache itself, all figure regeneration) are byte-deterministic given the seed — rerunning produces byte-identical CSVs. The *frontier-API* outputs (`repro-frontier` target) are *not* byte-deterministic because frontier-server batching, sampling-side nondeterminism, and model-version updates make exact CSV reproduction impossible; the published `results/frontier_*/results.csv` files are the canonical record for those models. Rerunning `repro-frontier` will produce closely-similar but not identical numbers.
...
| `caac_ablation.pdf` | no (Appendix) | The $\theta_q / N$ ablation plots that underpin §5.2.3 |
