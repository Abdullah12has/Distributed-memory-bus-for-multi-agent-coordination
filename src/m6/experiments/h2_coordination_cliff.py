    ...
    def _compute_verdicts(self, df: pd.DataFrame) -> dict[str, object]:
        ...
        for (c, family), sub in df.groupby(["compressor", "workload_family"]):
            ...
            if len(ratios) < 4 or float(np.ptp(success)) < 1e-6:
                continue
            fit = fit_piecewise(ratios, success)
            ...
            # stochastically larger than the above-τ distribution — i.e. that
            # crossing τ produces a downward shift.
        ...
        for cell, adj in zip(per_cell, adjusted, strict=False):
            cell["test_p_holm"] = adj
        ...
        return {
            ...
            "h2_supported": bool(
                df_fits["drop_rel"].ge(0.30).mean() >= 0.5 if not df_fits.empty else False
            ),
        }
