...
class H6Runner(ExperimentRunner):
    ...
    async def run(self) -> ExperimentResult:
        ...
        # Refuse to run silently on an empty signal set. Protected facts are
        # generated by family (a) [cross-doc fact aggregation] and family (c)
        # [multi-step retrieval]; family (b) [constraint satisfaction] does
        # not carry protected facts. If a config selects only family (b), H6
        # has nothing to measure — better to fail fast than to produce an
        # empty CSV and a useless verdict.
        n_with_facts = sum(1 for w in workloads if w.protected_facts)
        if n_with_facts == 0:
            msg = (
                "H6Runner refuses to start: 0 of "
                f"{len(workloads)} workloads carry protected_facts. The C1 "
                "generator emits protected facts in families (a) and (c) only; "
                "check configs/experiments/h6.yaml's workload_families list."
            )
            log.error("h6.no_signal", n_workloads=len(workloads))
            raise RuntimeError(msg)
        log.info("h6.workloads_with_facts", n=n_with_facts, n_total=len(workloads))
        ...
        for w in workloads:
            ...
            for condition, comp_name in (
                ...
                rows.append(
                    self.emit_row(
                        compressor=(comp_name or "none"),
                        ratio=4.0,
                        workload_family=w.family.value,
                        workload_id=w.workload_id,
                        seed=self.cfg.seeds[0],
                        metric=f"disclosure_{condition}",
                        value=recall,
                    )
                )
    ...
    async def _measure_disclosure(
        self, *, workload: Any, compressor_name: str | None, reader: Any
    ) -> float:
        ...
        for pf in workload.protected_facts:
            for q, expected in zip(pf.yesno_questions, pf.answers, strict=True):
                ...
                ans = (
                    completion.text.strip().lower().split()[0]
    ...
    def _verdict(self, df: pd.DataFrame) -> dict[str, object]:
        ...
        return {
            ...
            "c4_recall": c4,
            "metric_measures_real_signal": (
                baseline is not None and priors is not None and baseline > priors
            ),
