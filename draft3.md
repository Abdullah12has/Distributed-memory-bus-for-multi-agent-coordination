...
**C2 — Empirical characterisation of the coordination cliff and a compounding-error model that predicts it.** *The cliff exists, is sharp, and is invariant to planner parameter count and architecture family within a defined calibrated regime.* The empirical work covers four training-free compressors — LLMLingua-2 [Pan 2024], the Phi-3-Mini extractive span selector, an instruction-aware filter, and a truncation baseline — at ten compression ratios across three families and across three local planners drawn from **three different architecture families** (Qwen-2.5-1.5B-Instruct, Phi-3-Mini-3.8B-Instruct, Llama-3.1-8B-Instruct; the original "Qwen-1.5B/3.8B/8B" notation conflated three architectures and is corrected here), with a frontier validation arm on Qwen-2.5-72B-Instruct and DeepSeek V4 Pro that carries the corollary's binary verdict across the strict tolerance: Qwen-2.5-72B cliffs within $\sim 7\%$ of the canonical synthetic reference $\tau^{*}=2.5$ (and well inside the bootstrap CI on $\tau^{*}$), and DeepSeek V4 Pro's bootstrap CI contains the synthetic reference. (The local three-architecture sweep on family-c shows a $24\%$ tau spread, below the $50\%$ tolerance the validator originally shipped with but above the strict $20\%$ tolerance — necessary but not sufficient; the load-bearing evidence is the cross-architecture frontier arm. The local arm is itself a cross-architecture invariance result across Qwen, Phi-3, and Llama, not a within-family parameter-count sweep.) The compounding-error model derives the cliff position from per-round **critical-token-recall** and a per-family recall threshold $\theta_q$ and recovers the empirical position within a useful first-order tolerance. The same machinery supports **Corollary 2 (information-density scaling)**: the cliff *shape* varies with a task's information density $\theta_\text{info}$ (an AUC-based per-task quantity, distinct from $\theta_q$), validated on HotpotQA and MultiHopRAG against the C1 family-a synthetic reference. An out-of-regime diagnostic on GPT-oss 120B identifies extended-reasoning planners as a structurally distinct case that the model's threshold-success assumption does not cover — which is itself a contribution, since prior compression work treats reasoning regime implicitly.

**C3 — A catalogue of three retrieval-augmented-generation pipelines with a EUR-per-workflow cost model.** Three pipeline architectures (compress-first, retrieve-first, joint relevance-conditional routing) are implemented end-to-end on FAISS-CPU and HNSW and evaluated under matched storage-bounded and accuracy-bounded regimes. The original H3 hypothesis predicted a sign-flip in the optimal pipeline between regimes; the empirical work shows that compress-first (P1) is consistently preferred over retrieve-first (P2) in both regimes (by 3.2 pp F1 storage, 2.0 pp F1 accuracy), and the joint relevance-conditional routing (P3) leads both **by a substantial margin (42 pp F1 storage, 25 pp F1 accuracy)** — at near-identical EUR cost. The compress-first-over-retrieve-first finding *reverses* a common assumption of the LongLLMLingua [Jiang 2024] line of work, and the P3-over-both finding establishes the joint-routing variant as the headline performer at matched cost; both are reframings of the original sign-flip predicate that the empirical work invalidates.
...
**Chapter 4** presents the empirical work: H1, H2, the compounding-error model that predicts the cliff position with a predicted-versus-empirical match-rate band, Corollary 1 (cliff position is invariant to planner parameter count and architecture family within a defined calibrated regime, validated on two frontier models — Qwen-2.5-72B and DeepSeek V4 Pro — with GPT-oss 120B reported as an out-of-regime diagnostic at the regime boundary), H3 (the RAG pipeline catalogue with the compress-first dominance result), H4 (the inference-disclosure measurement on the unbiased benchmark), and Corollary 2 (the cliff *shape* varies with task information density across three benchmarks).
...
The Spearman correlation $p$-values in Table 4.1 below are from the two-tailed asymptotic test for $H_0: \rho = 0$; the displayed $n$ is the number of (workload, compression-ratio) measurements that enter the rank correlation after per-seed averaging — $n = 150 \text{ workloads} \times 9 \text{ compression ratios above the baseline}$ for the three compressors that sweep the full ratio range, and $n = 150 \times 5$ for Phi-3-Mini extractive because its ~$2.5\times$ achievable-compression ceiling collapses the upper half of the ratio sweep onto identical achieved compressions (see §3.2.3 for the ceiling discussion and Appendix~B for the per-cell row counts).
...
> **H1.** Single-agent question-answering accuracy under compression is a poor predictor of multi-fragment coordination success: Spearman $\rho(\Delta_\text{qa}, \Delta_\text{coord}) < 0.6$ on the cells of the cliff sweep, with 95% bootstrap CIs excluding 0.6 from above. **Verdict: SUPPORTED.** The headline empirical finding is **sign-disagreement across compressors**: the instruction-aware filter has $\rho = -0.59$ (strong *anti*-correlation — the re-ranker preserves task-keyword tokens that the F1 metric does not reward), while LLMLingua-2 and truncation have $\rho \approx +0.38$ (moderate positive — token-overlap F1 does track coordination success for compressors that drop tokens by token-level statistics). That two compressors disagree on the *sign* of the QA↔coordination relationship is the strictly stronger and more interesting finding than the preregistered $\rho<0.6$ threshold test, and it falsifies the use of token-overlap-F1 as a single-dimensional predictor of coordination quality across compressor families.
...
**Table 4.1 — H1 verdict (workload-level Spearman correlation between $\Delta$ token-overlap F1 and $\Delta$ coordination success). CI is BCa-bootstrap, 1000 resamples; $p$ is two-tailed asymptotic Spearman.**

| Compressor | $\rho$ | 95% CI (BCa) | $p$ (Spearman, two-tailed) | $n$ |
|---|---:|---|---|---:|
...
**Table 4.2 — H2 verdict (per-cell cliff position, piecewise-slope drop, and Holm-corrected paired-Wilcoxon $p$). The "slope drop" column is the piecewise-linear-extrapolated drop ratio $\Delta = (f(r_{\min}) - f(r_{\max})) / |f(r_{\min})|$ from the fit's slope-extrapolated boundary values, NOT a fraction of the observed coordination baseline; values $> 1$ occur because the left-side extrapolation can exceed the observed baseline-coordination of 1.0 by a small amount and the right-side extrapolation can go slightly below 0. The H2 verdict criterion (drop $\ge 0.30$) is in this same slope-extrapolated metric. For the observed-range drop (always $\le 1.0$), see Appendix B.**
...
The DeepSeek V4 Pro sweep returns a $\tau^{*}$ point estimate of 2.15 with a bootstrap CI of $[1.76, 7.14]$ — width $5.4$ ratio units, wider than the interesting region of the cliff sweep. The CI is wide because the sweep was run with the same small per-cell sample as Qwen-72B (10 workloads × 3 seeds per ratio) and DeepSeek's coordination curve happens to have more bootstrap-resample variance at the cliff position; the point estimate is below the canonical synthetic reference $\tau^{*}_\text{canonical} = 2.5$ but the CI contains it. The right framing is therefore: DeepSeek is consistent with Corollary 1 *with limited resolving power* — the CI is wide enough that it would also contain many other cliff positions, so the result is not a point-estimate match but a within-CI validation. The aggressive reading (DeepSeek cliffs slightly earlier than Qwen-72B because its critical-token-recall profile under LLMLingua-2 differs from Qwen's) is plausible and could be distinguished by sweeping DeepSeek with more seeds; the thesis takes the conservative reading.
...
Putting the three results together: the cliff position is invariant across **three local architecture families** (Qwen-2.5-1.5B, Phi-3-Mini-3.8B, Llama-3.1-8B), invariant within the Qwen-2.5 family across a **48× parameter scale-up** (1.5B local → 72B frontier — same family, very different sizes), and invariant across **two frontier architecture families** (Qwen-72B → DeepSeek V4 Pro) — all within the calibrated regime — and it is not invariant to reasoning regime (standard → extended). This is what an empirically-substantiated structural claim looks like: an invariance that holds across architecture families and parameter scales in a precisely-defined regime and visibly breaks at the regime boundary.
...
The compress-first preference over retrieve-first is itself a substantive finding because it reverses the assumed default of the post-retrieval-compression literature, and the P3-leading-both result is a stronger finding still. For practitioners, the recommendation is: **use joint relevance-conditional routing (P3) when the additional implementation cost is acceptable** — P3's F1 leads compress-first by 42 pp in the storage regime and 25 pp in the accuracy regime, at indistinguishable EUR-per-query cost ($\sim 3 \times 10^{-5}$ across all three pipelines); compress-first (P1) is the right default when implementation simplicity matters and the 25–42 pp F1 gap is not worth the integration cost; post-retrieval compression (P2) is only the right choice when the corpus is small enough that storage cost is irrelevant and retrieval recall is the binding constraint.
...
The cliff position is invariant across the three local architecture families tested (Qwen-2.5-1.5B, Phi-3-Mini-3.8B, Llama-3.1-8B) within a precisely-defined calibrated regime (§4.5) and is invariant within the Qwen-2.5 family across a 48× parameter scale-up (1.5B local → 72B frontier) and across vendors (Qwen → DeepSeek V4 Pro) within the frontier arm (§4.6). On the family-a × LLMLingua-2 cell, the Qwen-72B point estimate $\tau^{*}=2.68$ sits $\sim 7\%$ above the canonical synthetic reference $\tau^{*}=2.5$ and inside its bootstrap CI; the DeepSeek V4 Pro CI contains the same reference (with limited resolving power, see §4.6.3). The cliff is *not* invariant to reasoning regime: extended-reasoning planners such as GPT-oss 120B cliff at substantially higher ratios because they can recover from sub-threshold information by chain-of-thought, violating the threshold-success assumption A3. This is the empirical evidence that the cliff is a property of the (compressor, task) pair within a defined regime — the structural finding that the thesis contributes to the compression literature.
...
*Third*, compress-first (P1) is the better RAG architecture between the two compressed-corpus pipelines in both the storage-bounded and accuracy-bounded regimes (a finding that reverses the LongLLMLingua assumption); and joint relevance-conditional routing (P3) outperforms compress-first by **42 pp F1 (storage)** and **25 pp F1 (accuracy)** at indistinguishable EUR cost. The practitioner ranking is therefore P3 ≻ P1 ≻ P2 in both regimes, with the choice between P3 and P1 a function of implementation complexity, not cost.
