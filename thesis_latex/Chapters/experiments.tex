...
\textbf{Three hold as pre-specified.} H1 (workload-level QA-F1 does
...
outside the bootstrap band on \textbf{$0/11$ cells}: the band
...
cell, family-c~$\times$~LLMLingua-2: the model is conservative by
$\Delta\tau^{*}\approx +0.64$, which Corollary~2 explains via the
...
relevance-conditional routing) leads both on F1, but P3 routes
fragments \emph{verbatim} (achieved compression $1.00\times$) ---
so the cross-pipeline F1 gap is not a matched-compression
comparison and does \emph{not} support a Pareto claim for P3
(see Section~\ref{sec:h3_cost_caveat} for the cost-attribution caveat).
