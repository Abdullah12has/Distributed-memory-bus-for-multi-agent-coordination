...
\hline
LLMLingua-2 $\times$ a   & 2.5 & 1.61 & $8.5\!\times\!10^{-12}$ \\
LLMLingua-2 $\times$ b   & 2.5 & 1.61 & $8.5\!\times\!10^{-12}$ \\
LLMLingua-2 $\times$ c   & 6.7 & 1.23 & $9.6\!\times\!10^{-10}$ \\
Filter $\times$ a        & 2.5 & 1.60 & $8.5\!\times\!10^{-12}$ \\
Filter $\times$ b        & 2.5 & 1.61 & $8.5\!\times\!10^{-12}$ \\
Filter $\times$ c        & --- & 0.00 & n.s. \\
Phi-3-Mini ext.\ $\times$ a & 2.5 & 0.42 & $2.2\!\times\!10^{-6}$ \\
Phi-3-Mini ext.\ $\times$ b & 2.5 & 0.97 & $2.5\!\times\!10^{-9}$ \\
Phi-3-Mini ext.\ $\times$ c & 2.5 & 1.37 & $8.5\!\times\!10^{-12}$ \\
Truncation $\times$ a    & 2.5 & 1.61 & $8.5\!\times\!10^{-12}$ \\
Truncation $\times$ b    & 2.5 & 1.61 & $8.5\!\times\!10^{-12}$ \\
...
is $33\%$ within $\pm 25\%$ relative error and $67\%$ within $\pm 50\%$
...
bootstrap CI is constructed on $\theta_q$ by resampling workloads.
The predicted $\tau^{*}$ is then obtained by inverting
$q(r) = \theta_q$ on the empirical critical-token-recall curve.
Resampling $\theta_q$ through this inverse map gives a
\emph{predicted-$\tau^{*}$ band} that quantifies the model's
uncertainty.
...
$r\approx 6.7$, a $58\%$ relative under-prediction. The gap is
the second-order term that Corollary~2 explains
(Section~\ref{sec:corollary2}): family-c has lower information
density than family-a, so the threshold-success approximation
A2 (binary token importance) breaks earliest on family-c.
...
of $33\%$ ($4/12$ cells) at the
$|\tau^{*}_\text{empirical} - \tau^{*}_\text{predicted}| /
\tau^{*}_\text{empirical} \le 25\%$ criterion and $67\%$
($8/12$ cells) at $\pm 50\%$. The numbers are reported honestly:
the model is a useful first-order bound, not a tight predictor.
The four cells that pass the strict criterion span all four
compressors (lingua2/b, filter/a, truncation/a, truncation/b),
which is consistent with the bound being structural rather than
compressor-specific. The headline empirical claim it underwrites is the
...
sizes is $\tau^{*}_{1.5\text{B}} = 3.69$, $\tau^{*}_{3.8\text{B}}
= 4.68$, $\tau^{*}_{8\text{B}} = 4.01$ --- a spread of
approximately $24\%$ within the bootstrap CI of each cell, which is
what an invariance result looks like rather than a monotonicity
result. The original H5 predicate (gap $\ge 1.5$ ratio units) is
not satisfied because the spread is well below $1.5$; the
reframed Corollary~1 predicate (invariance within the calibrated
regime) is. The canonical results directory is
\texttt{results/h5\_final/}.
...
\subsection{Qwen-2.5-72B-Instruct: in-regime, \texorpdfstring{$0.8\%$}{0.8\%} off}
...
\hline
Instruction-aware filter & 0.50 & 0.78 & 0.57 & $+0.286\ (p_\text{Holm}\!=\!6\!\times\!10^{-4})$ & $-0.214\ (p_\text{Holm}\!=\!6\!\times\!10^{-4})$ \\
LLMLingua-2              & 0.50 & 0.78 & 0.59 & $+0.286\ (p_\text{Holm}\!=\!6\!\times\!10^{-4})$ & $-0.189\ (p_\text{Holm}\!=\!6\!\times\!10^{-4})$ \\
Phi-3-Mini extractive    & 0.50 & 0.78 & 0.71 & $+0.286\ (p_\text{Holm}\!=\!6\!\times\!10^{-4})$ & $-0.075\ (p_\text{Holm}\!=\!0.027)$ \\
\hline
