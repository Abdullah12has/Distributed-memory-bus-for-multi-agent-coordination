...
\section{Pre-registration and Refinement}
\label{sec:prereg_refinement}

The six investigations of this chapter were pre-registered with
falsifiable criteria in \texttt{configs/experiments/h\{1-6\}.yaml}
and \texttt{plan-v3.md}. The criteria for H3, H5, and H6 were
...
\textbf{Three hold as pre-registered.} H1 (workload-level QA-F1 does
not transferably predict coordination success), H2 (a sharp cliff
...
storage-bounded and an accuracy-bounded regime, with $\ge\!5$~pp
effect. \textbf{Verdict: not satisfied} --- no sign-flip is
observed. \textbf{Narrowed finding:} P1 (compress-first) is preferred
over P2 (retrieve-first) in \emph{both} regimes; P3 (joint
relevance-conditional routing) leads both. This does not reproduce,
on the single retriever/embedder stack tested here, the
post-retrieval-compression preference of the
LongLLMLingua~\cite{jiang2024longllmlingua} line of work.
\end{tcolorbox}
...
in both regimes; P3 leads both. The narrowed finding is the robust
...
--- MultiHopRAG cliffs at $11.3$, far from the C1 reference --- which
...
\hline
\multicolumn{4}{l}{\textbf{Tier A --- Holds as pre-registered.}} \\
...
H5 $\to$ Cor.~1 & Cliff-position invariance within calibrated regime (Ceiling--Cliff Separation) & SUPPORTED as \emph{no shift detected} (Llama-3.1-8B vs Qwen-2.5-72B match across $9\times$ scale + arch change; $\pm 20\%$ TOST not yet passed) & \texttt{h5\_final/}, \texttt{frontier\_qwen72b\_e2/}, \texttt{frontier\_deepseekv4/} \\
...
H3 & RAG sign-flip across regimes (pre-registered); compress-first dominance on the tested stack (refined) & Pre-registered predicate NOT satisfied; refined ordering SUPPORTED & \texttt{h3\_final/} \\
\hline
\end{tabular}
\end{table}
