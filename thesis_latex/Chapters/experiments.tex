...
bar is conservative against estimator noise: the empirical gaps
(Section~\ref{sec:corollary2}: $0.48$ between C1 family-a and
...
\begin{table}[h]
\centering
\caption{The verdict-critical evaluation metrics and the closest
alternative declined for each; every numerical claim in this chapter
resolves to one of these.}
\label{tab:metric_choices}
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm}>{\raggedright\arraybackslash}p{5.1cm}>{\raggedright\arraybackslash}p{4.3cm}}
\hline
Metric & Why chosen & Alternative declined \\
\hline
Coordination success (binary) & Directly answers ``did the planner recover the answer?'', the deployment question. & Graded success (future work; the A3 probe justifies binary on dense tasks). \\
Source-retention F1 (\texttt{qa\_f1}) & Comparable to the LLMLingua-2 / LongLLMLingua numbers, which is what makes the H1 mis-ranking claim meaningful. & ROUGE-L, BERTScore (less discriminating, or conflate compressor and metric). \\
Critical-token-recall (CTR) & Family-specific information-preservation signal; the model's $q(r)$ and CAAC's back-off signal. & Generic token-recall (misleading on Phi-3: keeps ``the'', drops numerics). \\
Spearman $\rho$, workload level & Rank correlation robust to the coordination $\{0,1\}$ boundaries; the H1 verdict statistic. & Pearson $r$ (needs linearity); pooled $\rho$ (pseudo-replication). \\
BCa bootstrap; paired Wilcoxon; Holm & Skew-robust CIs; a paired test for shared workloads; family-wise correction. & Percentile bootstrap; Mann--Whitney (independence fails); Bonferroni (over-conservative). \\
\hline
\end{tabular}
\end{table}
...
The deployment-relevant cliff for the family-a $\times$ LLMLingua-2
cell is the LLM-planner value $\tau^{*}\approx 2.7$ (local
Llama-3.1-8B $\approx 2.48$, frontier Qwen-2.5-72B $\approx 2.79$;
Section~\ref{sec:corollary1}); the coarse-grid deterministic-solver
value $2.5$ and its fine-grid re-resolution $\approx 1.1$ are
artefacts of the brittle regex solver, not deployment numbers.
...
retrieval, comparing three pipeline placements (compress-first,
retrieve-first, and a joint relevance-conditional router) under
matched storage- and accuracy-bounded regimes
(Section~\ref{sec:rag_pipelines}). The pre-registered prediction was
a sign-flip in the optimal pipeline between the two regimes; it is
\emph{not} observed. The narrowed, supported finding is that
compress-first is preferred over retrieve-first in both regimes (by
$3.2$ and $2.0$~pp F1, paired-bootstrap CIs excluding zero) on the
single \texttt{bge-large-en-v1.5} + FAISS-CPU stack tested, which
does \emph{not} reproduce the post-retrieval-compression preference
of the LongLLMLingua~\cite{jiang2024longllmlingua} line of work. A
joint router scores higher F1 but achieves it by routing fragments
verbatim, and the EUR cost model is corpus-dominated and does not
meter compression compute, so no cost-parity or Pareto claim is
made. Because this result is a single-stack non-reproduction
orthogonal to the coordination cliff and the memory bus, the full
verdict table, the pipeline definitions, and the cost-model caveat
are reported in Appendix~\ref{appendix:rag}.
