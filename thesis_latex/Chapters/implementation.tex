...
the reader enough detail to reproduce the system from the reference
...
\begin{tikzpicture}[
    ...
    ccmp/.style={draw, rounded corners=1pt, minimum width=1.7cm,
                 minimum height=0.6cm, font=\scriptsize, inner xsep=2pt},
...
% Compression layer
\node[layer, fill=black!4, minimum height=2.2cm] (compress) at (0,1.8) {};
\node[font=\small\bfseries]            at (0, 2.55) {Compression layer};
\node[font=\footnotesize\ttfamily]     at (0, 2.20) {Compressor protocol};
\node[ccmp] (id)     at (-4.75, 1.25) {identity};
\node[ccmp] (lingua) at (-2.85, 1.25) {LLMLingua-2};
\node[ccmp] (filter) at (-0.95, 1.25) {filter};
\node[ccmp] (phi)    at ( 0.95, 1.25) {Phi-3 ext.};
\node[ccmp] (trunc)  at ( 2.85, 1.25) {truncation};
\node[ccmp] (caac)   at ( 4.75, 1.25) {CAAC (wrap)};
% Storage layer
\node[layer, fill=black!4, minimum height=2.0cm] (storage) at (0,-1.0) {};
\node[font=\small\bfseries] at (0, -0.45) {Storage layer};
...
The instruction-aware filter is a pure-Python heuristic baseline operating in two stages. First, a sentence-level filter ranks sentences by relevance to the task hint using a cross-encoder reranker (\texttt{BAAI/bge-reranker-base}), falling back to lexical overlap when the model is unavailable. Second, a token-level trim drops English stop-words and short tokens ($\le 2$ characters) until the target ratio is reached; the stop-word list is an explicit ordered sequence so that the trim is fully deterministic given the task hint, the fragment, and the target ratio.
...
that uses an LLM at inference time. It runs Phi-3-Mini-3.8B~\cite{phi3}
via a local Ollama endpoint~\cite{ollama} with a structured prompt
requesting verbatim contiguous spans from the source. A post-hoc
verifier enforces extractiveness: a $15\%$ novel-token tolerance
accommodates common function-word insertions
(``the'', ``is'', ``and''), and any remaining novel tokens are
stripped before the output is accepted. Outputs that fail even after
stripping fall back to LLMLingua-2 at the same target ratio; in
practice this occurs only at aggressive ratios where span selection
cannot reach the target.

Phi-3-Mini extractive has a \emph{compression ceiling} of
approximately $2.5\times$, imposed by span-level selection and the
stripping pass together. This is documented as a limitation in
\charef{summary} and is the reason all evaluation reports include
the \emph{achieved} compression ratio alongside the requested target.
...
\texttt{CachedCompressor}
keyed on $(\text{compressor}, r, \text{fragment\_id}, \text{hash}(\text{task\_hint}))$)
...
Pro). All API calls are routed through the cost ledger so
...
\textbf{FAISS-CPU}~\cite{faiss} with an HNSW index (over Qdrant, Chroma,
or pgvector) so the reference service ships as one Docker image with no
external database and no GPU dependency for retrieval; open-weight BGE
embeddings with a BGE re-ranker; \textbf{FastAPI}/Starlette for the
access layer; \textbf{SQLite} in WAL mode for the audit log; \textbf{Ollama} for local LLM serving (one
\texttt{complete()} call gives a uniform model lifecycle across the
Apple-Metal and CUDA hosts, Section~\ref{sec:inference}); and the
