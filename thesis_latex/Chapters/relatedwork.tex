The four contributions build on seven bodies of prior work. Section~\ref{sec:rw_gap} consolidates the gap each contribution fills.

\section{The Transformer Architecture and the Cost of Context}
\label{sec:rw_transformer}

The large language models this thesis builds on are transformer-based~\cite{vaswani2017attention}. The transformer's defining
...
told practitioners how to spend \emph{training} compute productively
but said nothing about \emph{inference} cost, which is paid per
query for the model's deployment lifetime and dominates training
cost at production scale. Context compression is to
inference cost what scaling laws are to training cost: a structural
system that converts a linear cost into a constant one.

\section{The Scaling Era of LLMs}
\label{sec:rw_scaling}

Deployed model size grew four orders of magnitude from GPT-2
(1.5B, 2019) to the 2025--2026 frontier (Llama-3, Qwen-2.5,
DeepSeek V3/V4, Claude Sonnet 4.x, GPT-oss), each step raising
per-query cost and the demand for context-engineering. The two-to-three-order gap between frontier-cloud and
on-premises per-token cost is what makes compression an operational
concern. Context windows also grew in parallel (2K to multi-million
tokens), but a model's effective use of them lags the nominal
size (the ``lost in the middle'' effect~\cite{liu2024lostinmiddle,
hsieh2024ruler}), so tokens the model does not use are pure
...
\section{Training-Free Context Compression}
...
(xRAG~\cite{cheng2024xrag}). The four families and the design
dimensions that matter for multi-fragment evaluation are in
Table~\ref{tab:rw_comparison} at the end of this section.

\subsection{Token-level pruning (the LLMLingua family)}

The first context-compression technique widely adopted is
...
All three LLMLingua papers report
...
fragments. H1 of \charef{experiments} addresses that directly.
...
This literature's task-awareness is
...
will combine with other fragments, which is the gap the instruction-aware
...
Learned compression requires training,
...
Published guidance on enforcing the
...
The multi-agent LLM-systems literature expanded quickly after
...
These frameworks treat compression as an
implicit deployment optimisation; none measures the
\emph{multi-fragment} coordination-quality cost of compression
structurally. The cliff sweep and the disclosure measurement fill
that gap; the orthogonal multi-round-agent setting is left as future work.
...
\emph{et~al.}~\cite{lewis2020rag}. The notable later works
...
multi-fragment-coordination task structure presented in this thesis;
...
RAG benchmarks measure retrieval quality
...
CFedRAG~\cite{addison2024cfedrag} addresses coordination of federated
retrieval-augmented generation at the retrieval-index-level rather than
the compressor-level disclosure question that this thesis takes up.
...
No published work measures \emph{summary-level} inference disclosure: the
...
index, not the summary. This thesis closes that gap, and the policy
...
explicit in EUR per workflow. C3 is that catalogue, and its
honest finding is that the assumed sign-flip does not appear.
...
observation), and contributes the
