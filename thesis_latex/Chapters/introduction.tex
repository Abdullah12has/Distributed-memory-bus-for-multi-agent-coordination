...
Sam Altman informally estimated OpenAI's compute bill for processing
user politeness tokens alone at ``tens of millions of dollars'' a
year in 2025~\cite{altman2025pleasethankyou}.

Context compression is the direct lever. Per-token compute and cost
...
University of Oulu's main use case,
...
in any reported result: round-to-round LLM
variance dominates the compression signal at the C1 ratios swept, so
attributing a coordination drop to compression rather than agent
variance requires isolating the compressor on the critical
...
The four contributions (C1--C4) each address one sub-question,
operationalised by the hypotheses (H1--H6) in \charef{experiments}.
...
\paragraph{Headline.}
The single most-citable result is H1 (the
...
multi-fragment deployment. That finding drives the principal engineering deliverable: a \emph{compression-aware memory bus} (C4)
...
expense is dominated by token costs.
...
Claude Sonnet, Opus (Anthropic) and ChatGPT 5.5 (OpenAI) were used as coding assistants
throughout the implementation work for suggesting implementations, debugging
error messages, and code review. The author read, tested, and
modified each suggestion before adding it to the codebase. Experimental designs, evaluation
metrics, and result interpretations were not delegated to these tools.

\paragraph{Writing assistance.}
The author used AI language models as a proofreader: grammar checking,
sentence rephrasing, and \LaTeX\ formatting. All arguments, hypotheses,
findings, and conclusions are the author's own; no passage was generated
in full by an AI tool without the author's proofreading.

\paragraph{Literature search.}
AI tools were consulted during the literature survey to suggest candidate
papers and to surface connections across adjacent fields, including prompt
compression, multi-agent memory, and retrieval-augmented generation. Every
source cited was individually located and read by the author. The citation
list reflects the author's judgement on relevance, not an AI tool's.

\paragraph{Experimental reasoning.}
AI tools were used on occasion as a sounding board: for example, to check
whether a proposed statistical test suited the data, or to trace
interactions in a multi-component pipeline. All hypothesis formulations,
metric definitions, and result interpretations were designed by the author. 

\paragraph{Transparency note.}
This thesis studies context compressors and LLM planners belonging to the
same model class as the assistance tools above. The roles do not overlap.
Claude and ChatGPT appear in no experimental result; they were used only
for coding and writing assistance. The models accessed via the Featherless API and run locally are the experimental
planners; they were not used for assistance.
