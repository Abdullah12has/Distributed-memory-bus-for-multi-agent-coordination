...
misnaming was a name-versus-meaning drift of the kind that could
mislead a downstream reader. Lesson: metric-name versus
...
Figure~\ref{fig:predicted_vs_empirical} should therefore be read as
``the band is tight because $\theta_q$ is well-estimated; the model
is biased because A1--A4 are first-order''. The model's accuracy is
measured by the in-sample and LOO match rates, not by the band.
...
Llama-3.1-8B) on family-c shows a $\tau^{*}$ spread of $\approx 24\%$, below the \texttt{validate\_model\_independence} default $50\%$
