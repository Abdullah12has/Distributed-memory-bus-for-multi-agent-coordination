...
On the RAG-placement question, compress-first is preferred over
retrieve-first on the single retriever/embedder stack tested, but
the pre-specified regime sign-flip is not observed; because this is
a single-stack non-reproduction orthogonal to the cliff and the
bus, it is reported in Appendix~\ref{appendix:rag} rather than as a
headline result.
...
might not transfer to real ones. The within-family homogeneity is a
separate concern, which future work could resolve by sweeping
aggregation types within family-a.
...
Two caveats bound the no-shift claim. First, the local
cross-architecture sweep (Qwen-2.5-1.5B, Phi-3-Mini-3.8B,
Llama-3.1-8B) on family-c shows a $\tau^{*}$ spread of $\approx 24\%$,
below the \texttt{validate\_model\_independence} default $50\%$
tolerance but above the strict $\pm 20\%$ bar, so it is necessary
but not sufficient and the cross-architecture frontier evidence
(Section~\ref{sec:frontier}) carries the binary verdict. Second, the
predicted-$\tau^{*}$ bootstrap band (Section~\ref{sec:cem_match})
quantifies \emph{sampling} uncertainty in $\theta_q$ only, not the
model's \emph{specification} error, so the empirical $\tau^{*}$ falls
outside it on all $11$ testable cells; the in-sample and LOO match
rates, not the band, measure the model's accuracy
(Figure~\ref{fig:predicted_vs_empirical}).

\paragraph{A3 (threshold success): circularity broken by a direct probe.}
A3 was originally licensed only by the sharp cliff it explains,
which is a circular justification. The independent A3 probe
(Section~\ref{sec:a3_probe}, \texttt{results/a3\_probe}) breaks that
circularity: it deletes a controlled fraction of the task-critical
tokens \emph{without} a compressor and scores the remainder with an
LLM planner that \emph{could} reconstruct partial information. The
probe supports A3 directly on the dense aggregation family
...
statement is therefore that A3 is a threshold model for dense tasks
and a first-order approximation for distributed ones: the same
information-density axis Corollary~2 measures. A graded-success
refinement is the open direction. The cliff exists regardless; A3
describes its shape.
...
\texttt{results/tost\_corollary1.json}) shows that \emph{neither}
frontier cell passes. The point estimates match the reference well (Qwen-2.5-72B sits $\sim 7\%$ from the canonical $\tau^{*}=2.5$,
within $0.8\%$ of the auxiliary fit $2.6997$) but the $90\%$
bootstrap intervals on $\tau^{*}$ are too wide to rule out a
...
The work was scoped against two concrete industrial settings, and
each takes away a directly actionable result. \textbf{TalentAdore},
whose production workloads are dominated by per-token cost and whose
pricing calibrates the EUR-per-workflow cost model of this thesis
(\charef{implementation}~Section~\ref{sec:architecture}), gains four
things. First, the H1 finding is a procurement guardrail: a
compressor must \emph{not} be chosen on the single-agent QA-F1
numbers the prompt-compression literature reports (LongBench-,
RULER-, or LLMLingua-style), because those numbers do not predict
multi-fragment coordination success (Section~\ref{sec:synthesis});
a multi-fragment metric such as the C1 coordination score is
required. Second, the measurement loop is cheap to run in
production: measure $\theta_q$ once on a small in-domain calibration
set, read a safe operating ratio off the compounding-error model,
and avoid a full per-workload ratio sweep. Third, compress-first
(P1) is the safe default retrieval pipeline on a comparable stack.
Fourth, the summary-level disclosure metric and the memory-bus
policy layer together give a deployment-time governance budget for
the \textsc{confidential} candidate data TalentAdore processes, and
CAAC provides a drop-in, recall-bounded safety floor for any inner
compressor already in use.

\textbf{FCG} (the Future Computing Group) gains a runnable reference
artefact: the three-layer memory bus extends the group's
MemIndex~\cite{saleh2025memindex} baseline, ships as a single
\texttt{docker-compose} image, and is the integration target for the
group's multi-agent roadmap. The flagship \emph{Vignette~3.7: period-end project reporting} use case~\cite{fcgusecase2026}, which
aggregates reporting data across roughly eight institutional systems
via about eight specialised agents, is exactly the multi-fragment
workflow this thesis characterises; the internal FCG analysis
estimates an $80\%$ cloud-payload reduction at approximately
EUR~$2.15$ per report. The cliff characterisation tells FCG
\emph{how aggressively that payload can be compressed before the
cross-system aggregation breaks} (the question the $80\%$ figure
cannot answer on its own) and the disclosure metric indicates
which compressor to route classified institutional fragments
through. The bus's distributed and multi-tenant behaviour
(contention under concurrent writers, policy-lattice scaling,
cross-host replication) is not measured here and is the natural FCG
...
Third, on the RAG-placement question, prefer compress-first over
retrieve-first on a comparable stack; the joint router (P3) scores
higher F1 only by routing fragments verbatim, so its
cost-versus-quality trade-off must be measured before deployment.
This is a secondary, single-stack result reported in
Appendix~\ref{appendix:rag}, not a headline practitioner
recommendation.
...
reports~\cite{anthropic2025contextengineering, anthropic2025contextediting}) is consistent with this thesis and underwrites the practitioner
...
frontier sample would stabilise the piecewise fit under resampling (the DeepSeek bootstrap median $5.7$ sitting far from its point
